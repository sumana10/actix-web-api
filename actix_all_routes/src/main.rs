...
#[derive(Deserialize)]
//Option<T> means the value might be there or might not be
struct UserQuery {
    page: Option<u32>,
    limit: Option<u32>,
...
async fn home() -> Result<HttpResponse> {
    Ok(HttpResponse::Ok().json(serde_json::json!({"message": "Welcome to API"})))
}

async fn health() -> Result<HttpResponse> {
    Ok(HttpResponse::Ok().json(serde_json::json!({"message": "Healthy"})))
}
...
// 4. Query Parameters
//.as_deref() converts Option<String> to Option<&str> — it borrows the string inside instead of moving
//Then .unwrap_or("") returns the inner &str if Some, or an empty string "" if None.
async fn get_users(query: web::Query<UserQuery>) -> Result<HttpResponse> {
    ...
    let users = vec![
        User {
            id: 1,
            name: format!("User1-{}, search", search),
...
async fn get_user_post(path: web::Path<(u32, u32)>) -> Result<HttpResponse> {
    let (user_id, post_id) = path.into_inner(); // <- This is tuple destructuring
    let post: Post = Post {
        id: post_id,
